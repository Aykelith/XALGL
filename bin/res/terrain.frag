...
void main()
{
    ...
    vec3 result = CalcDirLight(dirLight, norm, viewDir, totalColor.rgb);
    // Phase 2: Point lights
    for(int i = 0; i < 2; i++)
        result += CalcPointLight(pointLights[i], norm, FragPos, viewDir, totalColor.rgb);    
    // Phase 3: Spot light
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir, vec3 color)
{
    ...
    vec3 specular = light.specular * spec * vec3(0.f, 0.f, 0.f);
